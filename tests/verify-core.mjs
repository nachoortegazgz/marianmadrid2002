...
check('Esquema CMS canónico cubre las colecciones y los índices activos del runtime', () => {
  ...
  const serviceCatalog = schemaById.get('Import2');
  for (const fieldId of ['personalDisponible', 'bookingsSyncEnabled', 'bookingsCategoryId']) {
    assert.ok(serviceCatalog.fields.some(([candidate]) => candidate === fieldId), `Import2 no declara ${fieldId}`);
  }
  const mapaStaff = schemaById.get('MapaStaff');
  assert.ok(mapaStaff);
  for (const fieldId of ['resourceId', 'memberId', 'email', 'scheduleId', 'nombreVisible', 'rol', 'activo']) {
    assert.ok(mapaStaff.fields.some(([candidate]) => candidate === fieldId), `MapaStaff no declara ${fieldId}`);
  }
...
check('Sincronizacion de servicios usa cola, revision nativa y errores saneados', () => {
  const config = read('src/backend/internalConfig.js');
  const hooks = read('src/backend/data.js');
  const sync = read('src/backend/bookingsServiceSync.js');
  const crons = read('src/backend/crons.js');
  const schemaById = new Map(canonicalSchema.collections.map((collection) => [collection.id, collection]));
  const queue = schemaById.get('BookingsServiceSyncQueue');
  assert.ok(config.includes('BOOKINGS_SERVICE_SYNC_QUEUE'));
  assert.ok(queue);
  for (const fieldId of ['serviceId', 'sourceItemId', 'desiredHash', 'status', 'attempts', 'nextAttemptAt', 'traceId', 'errorCode']) {
    assert.ok(queue.fields.some(([candidate]) => candidate === fieldId), fieldId);
  }
  for (const token of ['bookingsSyncEnabled', 'enqueueBookingsServiceSync', 'Import2_afterInsert', '_enqueueBookingsServiceSyncSafely']) assert.ok(hooks.includes(token), token);
  const staff = read('src/backend/staff.js');
  assert.ok(sync.includes('resolveStaffResourceIds'));
  for (const token of ['wixData', 'COLLECTIONS.MAPA_STAFF', 'resolveStaffResourceIds', 'clearStaffCache']) assert.ok(staff.includes(token), token);
  assert.equal(staff.includes('wix-secrets-backend'), false);
  assert.equal(staff.includes('SECRETS.MAPA_STAFF'), false);
  for (const token of ['getServiceElevated', 'updateServiceElevated', 'revision', 'BOOKINGS_PERMISSION_DENIED', 'BOOKINGS_SYNC_FAILED', 'processBookingsServiceSyncQueue']) assert.ok(sync.includes(token), token);
  assert.equal(sync.includes('error?.message || String(error)'), false);
